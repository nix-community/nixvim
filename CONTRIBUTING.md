...
| **callSetup**                | Default behavior for the generated setup call. Accepts `true`, `false`, or `"optional"`. `"optional"` emits the call only when the user defines `settings`. An explicit `settings = { }` still emits it. Use `"optional"` when `setup()` is only needed for customizations. Modules with Lua configuration expose a boolean `plugins.<name>.callSetup` or `colorschemes.<name>.callSetup` override whose default follows this built-in behavior. A forced call can emit a duplicate or invalid `setup()` for plugins that generate custom setup code. | No       | `true`                                                                                          |
