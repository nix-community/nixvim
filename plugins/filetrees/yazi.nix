...
lib.nixvim.neovim-plugin.mkNeovimPlugin config {
  ...
  description = ''
    ...
    The default grep functionality requires `telescope`.
    You can override the grep functionality through the `plugins.yazi.settings.integrations.grep_in_directory`
    and `plugins.yazi.settings.integrations.grep_in_selected_files` options
    if you do not wish to use `telescope`.
    Otherwise you must enable `plugins.telescope` or install it some other way.
  '';
