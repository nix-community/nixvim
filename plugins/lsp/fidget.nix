...
in {
    ...
    plugins.fidget =
      ...
      // {
        ...
        progress = {
          ...
          display = {
            ...
            overrides =
              ...
              (
                with types;
                  attrsOf notificationConfigType
              )
              ''
                {
                  rust_analyzer = {
        ...
        notification = {
          ...
          filter = helpers.defaultNullOpts.mkEnum ["off" "error" "warn" "info" "debug" "trace"] "info" ''
          ...
          configs =
            ...
            (
              with types;
                attrsOf (
                  either notificationConfigType str
                )
            )
            ''
              {
          ...
          window = {
            ...
            align = helpers.defaultNullOpts.mkEnum ["top" "bottom" "avoid_cursor"] "bottom" ''
              How to align the notification window.
            '';

            relative = helpers.defaultNullOpts.mkEnumFirstDefault ["editor" "win"] ''
        ...
        logger = {
          level = helpers.defaultNullOpts.mkEnum ["off" "error" "warn" "info" "debug" "trace"] "warn" ''
  ...
  config = mkIf cfg.enable {
    ...
    extraConfigLua = let
      processNotificationConfig = notificationConfig:
          ...
          notification = with notification; {
            ...
            filter =
              ...
              (
                if (isInt filter)
                then filter
                else helpers.mkRaw "vim.log.levels.${strings.toUpper filter}"
              );
            history_size = historySize;
            override_vim_notify = overrideVimNotify;
            configs =
              helpers.ifNonNull' configs
              (
                mapAttrs
                (
                  _: value:
                    if isString value
                    then helpers.mkRaw value
                    else processNotificationConfig value
                )
                configs
              );
            redirect =
