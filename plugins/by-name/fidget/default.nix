  ...
  settingsOptions = {
    progress = {
      ...
      display = {
        ...
        progress_ttl = defaultNullOpts.mkNum { __raw = "math.huge"; } ''
    ...
    notification = {
      ...
      configs =
        defaultNullOpts.mkAttrsOf (with types; either str notificationConfigType)
    ...
    logger = {
      ...
      float_precision = defaultNullOpts.mkNum 0.01 ''
        Limit the number of decimals displayed in floating point numbers.
      '';

      path =
        defaultNullOpts.mkStr { __raw = ''string.format("%s/fidget.nvim.log", vim.fn.stdpath("cache"))''; }
          ''
