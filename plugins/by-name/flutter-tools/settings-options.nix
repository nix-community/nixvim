...
in
{
  ...
  debugger = {
    ...
    exception_breakpoints = defaultNullOpts.mkListOf types.str null ''
  ...
  flutter_lookup_cmd = defaultNullOpts.mkStr' {
    ...
    description = ''
      A command to find the flutter installation location.
    '';
  };
  ...
  widget_guides = {
    enabled = defaultNullOpts.mkBool false ''
      Whether to enable the widget guides.
    '';
  };
  ...
  dev_log = {
    ...
    open_cmd = defaultNullOpts.mkStr' {
      example = "15split";
      pluginDefault.__raw = "('botright %dvnew'):format(math.max(vim.o.columns * 0.4, 50))";
      description = ''
  ...
  outline = {
    open_cmd = defaultNullOpts.mkStr' {
      pluginDefault.__raw = "('botright %dvnew'):format(math.max(vim.o.columns * 0.3, 40))";
      example = "30vnew";
      description = ''
